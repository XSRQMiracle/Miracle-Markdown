...
// including SVG generated by the installed MathJax package.
...
eq([lines[1].fill, lines[1].stroke, lines[1].strokeWidth], [null, null, 70], "MathJax table rule sets width without overriding stroke: none");
...

{
  ...
  eq(strokes.some((event) => event.width === 70), false, "canvas leaves the explicitly disabled table separator unpainted");
...
// Use real TeX output so the SVG fixture cannot invent stylesheet semantics.
// MathJax's Node adaptor supplies the generated attributes and nesting; only
// the browser DOM interface and canvas calls are replaced for this test.
{
  ...
  const adapt = (node: LiteElement): FakeElement => element(
    node.kind,
    Object.fromEntries(adaptor.allAttributes(node).map(({ name, value }: { name: string; value: string }) => [name, value])),
    node.children.filter((child): child is LiteElement => child.kind !== "#text").map(adapt),
    adaptor.textContent(node),
  );
  const source = String.raw`{\color{red}\begin{array}{|c|c|}\hline a&b\\\hline c&d\\\hline\end{array}}+e`;
  const generatedSvg = adapt(adaptor.firstChild(document.convert(source, { display: true })));
  eq(generatedSvg.querySelector('[data-mml-node="merror"]'), null, "the colored array parses in real MathJax");
  eq(generatedSvg.querySelector("[data-frame]") !== null, true, "real MathJax emits an array frame");
  eq(generatedSvg.querySelector("[data-line]") !== null, true, "real MathJax emits array separators");
  const generated = geometryFromSvg(generatedSvg as unknown as SVGSVGElement).commands ?? [];
  const rules = generated.filter((command) => command.kind === "path" && ["line", "rect"].includes(command.source));
  eq(rules.map((command) => [command.fill, command.stroke, command.strokeWidth]),
    [[null, "red", 70], [null, "red", 70], [null, "red", 70]],
    "real array separators and frame preserve the inherited red stroke");
  ...
  replay.call({ ctx: context }, generated, "#123456");
  eq(context.events.filter((event) => event.op === "stroke" && event.width === 70).map((event) => event.color),
    ["red", "red", "red"], "canvas paints real array separators and frame in their inherited color");
  eq(context.events.filter((event) => event.op === "fill").map((event) => event.color),
    ["red", "red", "red", "red", "#123456", "#123456"],
    "canvas preserves explicit glyph colors and resolves the surrounding run color");
}

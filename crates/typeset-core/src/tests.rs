...
fn lines_of(text: &str, width: f32, cfg: Config) -> Vec<String> {
    ...
    layout_lines(&para, &breaks, width)
        .iter()
        .map(|l| {
            l.runs
                .iter()
...
fn no_mixed_script_spacing_when_disabled() {
    let cfg = Config { font_size: EM, cjk_latin_spacing: false, ..Default::default() };
    let para = build("中abc文", cfg);
...
fn every_run_maps_back_to_its_source_bytes() {
    ...
    let lines = layout_lines(&para, &breaks, EM * 20.0);
...
fn handles_degenerate_input_without_panicking() {
    ...
    for text in ["", " ", "a", "。", "中", "\u{200B}", &"x".repeat(500)] {
        ...
        for width in [1.0, EM, EM * 3.0, 10_000.0] {
            let breaks = break_lines(&para, width);
            let _ = layout_lines(&para, &breaks, width);
        }
    }
}
...
fn ragged_right_does_not_stretch_the_glue() {
    ...
    let justified = {
        let cfg = Config { font_size: EM, justify: true, ..Default::default() };
        let para = build(text, cfg);
        let breaks = break_lines(&para, width);
        layout_lines(&para, &breaks, width)
    };
    let ragged = {
        let cfg = Config { font_size: EM, justify: false, ..Default::default() };
        let para = build(text, cfg);
        let breaks = break_lines(&para, width);
        layout_lines(&para, &breaks, width)
    };
...
fn font_expansion_scales_glyphs_instead_of_spaces() {
    ...
    let width = EM * 17.0;
    let cfg = Config { font_size: EM, max_expand: 0.02, ..Default::default() };
    let para = build(text, cfg);
    let breaks = break_lines(&para, width);
    let lines = layout_lines(&para, &breaks, width);
    let scaled = lines
...
fn syllables_of_one_word_are_positioned_without_gaps() {
    ...
    let lines = layout_lines(&para, &breaks, EM * 40.0);
